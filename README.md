<div align="center">

<img src="https://capsule-render.vercel.app/api?type=waving&color=0:4ade80,50:60a5fa,100:f472b6&height=200&section=header&text=Backend%20Challenges&fontSize=52&fontColor=ffffff&animation=fadeIn&fontAlignY=38&desc=Desafios%20práticos%20do%20Júnior%20ao%20Sênior&descAlignY=58&descSize=18&descColor=ffffff"/>

**Desafios práticos de backend para todos os níveis.**  
Resolva em qualquer linguagem. Aprenda. Mostre no portfólio.

---

![Desafios](https://img.shields.io/badge/desafios-6%2B4%20planejados-4ade80?style=for-the-badge)
![Níveis](https://img.shields.io/badge/níveis-júnior%20%7C%20pleno%20%7C%20sênior-facc15?style=for-the-badge)
![Linguagens](https://img.shields.io/badge/linguagens-agnóstico-60a5fa?style=for-the-badge)
![Licença](https://img.shields.io/badge/licença-MIT-f472b6?style=for-the-badge)

![GitHub Stars](https://img.shields.io/github/stars/Francisco-Montalvao/backend-challenges?style=social)
![GitHub Forks](https://img.shields.io/github/forks/Francisco-Montalvao/backend-challenges?style=social)
![Contributors](https://img.shields.io/github/contributors/Francisco-Montalvao/backend-challenges)
![Last Commit](https://img.shields.io/github/last-commit/Francisco-Montalvao/backend-challenges/main)

[![Ver Website](https://img.shields.io/badge/🌐%20Ver%20Website-GitHub%20Pages-blue?style=for-the-badge)](https://francisco-montalvao.github.io/backend-challenges/)

</div>

---

## 🌎 English Summary

Backend practice challenges in Portuguese that you can solve in any language.  
Each challenge has three parts: a full spec, hints for when you get stuck, and a community solutions page.  
To share a solution, fork the repo, add your entry to the challenge's `SOLUTIONS.md`, and open a PR.

---

## 🤔 Por que esse repositório existe?

Processos seletivos pedem desafios técnicos. Devs querem praticar. Mas repositórios com desafios de backend bem estruturados — com contexto real, critérios claros e dicas de estudo — são raros.

Esse repositório resolve isso. Cada desafio simula um problema real de mercado e acompanha recursos para você aprender enquanto resolve.

**Você resolve na linguagem que quiser.** Java, Python, Go, Node.js, C#, Rust — sem restrições.

---

## 🗂️ Como cada desafio funciona

Todo desafio tem três partes — cada uma com um propósito claro:

| Arquivo | O que tem dentro |
|---------|-----------------|
| `README.md` | O enunciado completo: modelo de dados, endpoints e exemplos |
| `dicas.md` | Onde trava 90% das pessoas. Leia quando ficar preso — sem spoilers da solução |
| `SOLUTIONS.md` | O que a comunidade já resolveu. Compare abordagens, inspire-se |

---

## 🗺️ Desafios

### 🟢 Júnior — Fundamentos Sólidos
> CRUD, validação, autenticação básica e boas práticas de API REST.

| Desafio | Tema | Enunciado | Dicas | Soluções |
|---------|------|-----------|-------|----------|
| 📝 To-Do List API | CRUD | [Ver desafio](./junior/crud/todo-list/README.md) | [Dicas](./junior/crud/todo-list/dicas.md) | [Soluções](./junior/crud/todo-list/SOLUTIONS.md) |
| 🔐 Login com JWT | Autenticação | [Ver desafio](./junior/autenticacao/login-jwt/README.md) | [Dicas](./junior/autenticacao/login-jwt/dicas.md) | [Soluções](./junior/autenticacao/login-jwt/SOLUTIONS.md) |
| 📦 Gerenciador de Produtos | CRUD Avançado | [Ver desafio](./junior/crud/gerenciador-produtos/README.md) | [Dicas](./junior/crud/gerenciador-produtos/dicas.md) | [Soluções](./junior/crud/gerenciador-produtos/SOLUTIONS.md) |
| 📊 Relatório de Vendas | Agregações | [Ver desafio](./junior/relatorios/vendas/README.md) | [Dicas](./junior/relatorios/vendas/dicas.md) | [Soluções](./junior/relatorios/vendas/SOLUTIONS.md) |

### 🟡 Pleno — Sistemas Avançados
> Mensageria, cache, performance e autenticação avançada.

| Desafio | Tema | Enunciado | Dicas | Soluções |
|---------|------|-----------|-------|----------|
| 📨 Fila de Notificações | Mensageria | [Ver desafio](./pleno/mensageria/fila-notificacoes/README.md) | [Dicas](./pleno/mensageria/fila-notificacoes/dicas.md) | [Soluções](./pleno/mensageria/fila-notificacoes/SOLUTIONS.md) |

### 🔴 Sênior — Arquitetura Distribuída
> Microsserviços, resiliência, padrões avançados.

| Desafio | Tema | Enunciado | Dicas | Soluções |
|---------|------|-----------|-------|----------|
| 🏗️ Sistema de Pedidos | Microsserviços | [Ver desafio](./senior/microsservicos/sistema-pedidos/README.md) | — | [Soluções](./senior/microsservicos/sistema-pedidos/SOLUTIONS.md) |

---

> ✅ **6 desafios disponíveis** · 🚧 [**Em breve: OAuth2, Rate Limiting, GraphQL, Cache com Redis →**](./docs/ROADMAP.md)

---

## 🧭 Por onde começar?

| Se você... | Vá para |
|-----------|---------|
| É novo em APIs REST | [📝 To-Do List](./junior/crud/todo-list/README.md) — CRUD básico, sem complicação |
| Quer praticar autenticação | [🔐 Login com JWT](./junior/autenticacao/login-jwt/README.md) |
| Já sabe CRUD e quer algo a mais | [📦 Gerenciador de Produtos](./junior/crud/gerenciador-produtos/README.md) |
| Quer aprender mensageria | [📨 Fila de Notificações](./pleno/mensageria/fila-notificacoes/README.md) |
| Quer um desafio sério de arquitetura | [🏗️ Sistema de Pedidos](./senior/microsservicos/sistema-pedidos/README.md) |

---

## ⚡ Quick Start

```bash
# Clone o projeto
git clone https://github.com/Francisco-Montalvao/backend-challenges.git
cd backend-challenges

# Entre no desafio recomendado para iniciantes
cd junior/crud/todo-list

# Leia o enunciado e comece!
cat README.md
```

---

## 💡 Como compartilhar sua solução

1. Faça um **fork** do repositório
2. Edite o `SOLUTIONS.md` do desafio que você resolveu — adicione uma linha com seu nome, linguagem e link do repositório
3. Abra um **Pull Request**

> 🌟 **As primeiras soluções da comunidade ainda não chegaram — a sua pode ser a primeira!**

Dúvidas? [GitHub Discussions →](https://github.com/Francisco-Montalvao/backend-challenges/discussions)

---

## 📚 Documentação

| Recurso | Descrição |
|---------|-----------|
| [🗺️ Estrutura](./docs/ORGANIZACAO.md) | Organização do projeto |
| [🚀 Roadmap](./docs/ROADMAP.md) | Plano para 2026-2027 |
| [🤝 Comunidade](./docs/COMMUNITY.md) | Como participar e contribuir |

---

## 📜 Licença

MIT — use, adapte, compartilhe. Só não esquece de deixar a ⭐ se te ajudou!

---

<div align="center">

<img src="https://capsule-render.vercel.app/api?type=waving&color=0:f472b6,50:60a5fa,100:4ade80&height=100&section=footer"/>

**Feito com 💙 para a comunidade dev brasileira**

</div>